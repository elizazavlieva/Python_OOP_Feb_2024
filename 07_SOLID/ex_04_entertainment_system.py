...
class Television(RCAMixin, HDMIMixin, PowerMixin):
...
class DVDPlayer(HDMIMixin, PowerMixin):
...
class GameConsole(HDMIMixin, EthernetMixin, PowerMixin):
    def connect_to_tv(self, television):
        self.connect_to_device_via_hdmi_cable(television)
...
class Router(EthernetMixin, PowerMixin):
    def connect_to_tv(self, television):
